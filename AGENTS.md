...
- **The version lives in three Claude places and must match**: `.claude-plugin/plugin.json`, `MG_VERSION` in `scripts/lib.sh`, the top entry of `CHANGELOG.md`; plus four Codex places: `.codex-plugin/plugin.json`, `model_guard/__init__.py`, `pyproject.toml`, `native/release.json` (`plugin_version` and the release URL). `check-install.sh` decides whether to refresh a user's installed scripts by comparing `MG_VERSION` — a release that bumps plugin.json but not `MG_VERSION` leaves every user on the old scripts, silently.
