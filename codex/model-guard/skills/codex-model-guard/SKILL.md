...
The installer creates a private Python environment and adds a managed PATH block to bash/zsh/Fish startup files (Fish respects `XDG_CONFIG_HOME` and does not change persistent universal variables). It preserves the official executable, model defaults, provider configuration and login files. After installation, run `~/.local/share/model-guard-codex/bin/model-guard-codex doctor`; if `MODEL_GUARD_CODEX_HOME` is set, use that directory instead. A new terminal picks up the launcher, then the user runs `codex` normally. Existing sessions continue as they are.
